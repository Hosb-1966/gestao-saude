...
function AppContent() {
  ...
  const [cardTitles, setCardTitles] = useLocalStorage('card-titles', {
    ...
    exames: 'Exames', // CORRIGIDO: Alterado de "hoje" para "exames"
    unidades: 'Unidades'
  });
  ...
  // CORRIGIDO: Cálculos automáticos baseados nas tabelas (useMemo para otimização)
  const calculatedStats = useMemo(() => {
    // CORRIGIDO: Total de médicos únicos das consultas (apenas médicos com agendas)
    ...
    // CORRIGIDO: Total de exames únicos (apenas tipos de exames únicos)
    ...
    return {
      totalMedicos: medicosConsultas.size, // CORRIGIDO: Apenas médicos únicos das consultas
      totalRemarcacoes,
      totalExames: tiposExamesUnicos.size, // CORRIGIDO: Exames únicos em vez de "hoje"
  ...
  const remarcacoesStats = useMemo(() => {
    ...
    // CORRIGIDO: Médicos com remarcações "Dentro" e "Fora" para gráfico empilhado
  ...
  // CORRIGIDO: Dados para gráfico empilhado de médicos com Dentro/Fora
  ...
  // CORRIGIDO: Função de exportação Dashboard PDF com dados tabulares
  const handleExportDashboardPDF = async () => {
    const timestamp = new Date().toISOString().slice(0, 19).replace(/:/g, '-');
    
    // Criar conteúdo HTML para PDF com dados dos gráficos
    const htmlContent = `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body>
        ...
        <div class="chart-section">
          <div class="chart-title">Médicos por Prazo de Remarcação</div>
          <table>
            <tr><th>Médico</th><th>Dentro do Prazo</th><th>Fora do Prazo</th><th>Total</th></tr>
            ${medicosEmpilhadoData.map(item => `<tr><td>${item.fullName}</td><td>${item.Dentro}</td><td>${item.Fora}</td><td>${item.Dentro + item.Fora}</td></tr>`).join('')}
          </table>
        </div>
    ...
    a.href = url;
    a.download = `dashboard-dados-${timestamp}.html`;
    a.click();
  ...
  // Rodapé com instruções mais concisas e ícones
  const Footer = () => (
  ...
  const Dashboard = () => (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* CORRIGIDO: Gráfico "Agendas por Unidade" com altura e largura aumentadas proporcionalmente */}
      ...
      {/* CORRIGIDO: Novo gráfico empilhado de médicos com Dentro/Fora */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <Card className="border-slate-200 shadow-sm">
          <CardHeader>
            <CardTitle className="text-lg font-medium text-slate-700 flex items-center gap-2">
