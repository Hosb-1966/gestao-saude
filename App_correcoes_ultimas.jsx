...
import { BarChart3, Calendar, Stethoscope, Repeat, Users, Clock, MapPin, Edit2, Save, X, Plus, Building2, Trash2, Phone, Timer, UserCheck, AlertTriangle, Download, FileText, BarChart2, Activity, Settings } from 'lucide-react';
...
function AppContent() {
  ...
  // CORRIGIDO: Atividade semanal calculada automaticamente das tabelas
  const atividadeSemanelCalculada = useMemo(() => {
    ...
    return diasSemana.slice(0, 6).map(dia => {
      ...
      return {
        ...
        agendas: totalAgendas // CORRIGIDO: "agendamentos" → "agendas"
      };
    });
  }, [agenda, exames]);
  ...
  const remarcacoesStats = useMemo(() => {
    ...
    // Médicos que mais solicitaram remarcações (ofensores)
  ...
  // Dados para gráfico de prazo (apenas "Dentro" e "Fora") - ÁREA AUMENTADA
  ...
  // CORRIGIDO: Top 5 médicos ofensores - BARRAS HORIZONTAIS
  const topOfensores = Object.entries(remarcacoesStats.medicosRemarcacoes)
    .sort(([,a], [,b]) => b - a)
    .slice(0, 5)
    .map(([medico, count]) => ({ 
      name: medico.length > 15 ? medico.substring(0, 15) + '...' : medico, 
      value: count,
      fullName: medico
    }));
  ...
  // CORRIGIDO: Função de exportação PDF
    ...
    const htmlContent = `
      ...
      <body>
        ...
        <div class="stats">
          ...
          <div class="stat-card">
            <h3>${cardTitles.hoje}</h3>
            <p>${calculatedStats.agendamentosHoje}</p>
          </div>
          <div class="stat-card">
        ...
        <div class="chart-section">
          <div class="chart-title">Métricas de Remarcações</div>
          <table>
            <tr><th>Métrica</th><th>Valor</th></tr>
            ${remarcacoesPorMetrica.map(item => `<tr><td>${item.nome}</td><td>${item.valor}${item.nome.includes('Horas') ? 'h' : ''}</td></tr>`).join('')}
          </table>
        </div>

        <div class="chart-section">
          <div class="chart-title">Meios de Contato</div>
          <table>
            <tr><th>Tipo</th><th>Quantidade</th><th>Percentual</th></tr>
            ${contatoData.map(item => `<tr><td>${item.tipo}</td><td>${item.count}</td><td>${item.percentage}%</td></tr>`).join('')}
          </table>
        </div>
        ...
        <div class="chart-section">
          <div class="chart-title">Prazo das Remarcações</div>
          <table>
            <tr><th>Status</th><th>Quantidade</th><th>Percentual</th></tr>
            ${prazoData.map(item => `<tr><td>${item.name}</td><td>${item.value}</td><td>${item.percentage}%</td></tr>`).join('')}
          </table>
        </div>

        <div class="chart-section">
          <div class="chart-title">Agendas por Unidade</div>
          <table>
            <tr><th>Unidade</th><th>Consultas</th><th>Exames</th><th>Total</th></tr>
            ${agendaUnificada.map(item => `<tr><td>${item.nome}</td><td>${item.consultas}</td><td>${item.exames}</td><td>${item.consultas + item.exames}</td></tr>`).join('')}
          </table>
        </div>

        <div class="chart-section">
          <div class="chart-title">Top Médicos Ofensores</div>
          <table>
            <tr><th>Médico</th><th>Remarcações</th></tr>
            ${topOfensores.map(item => `<tr><td>${item.fullName}</td><td>${item.value}</td></tr>`).join('')}
          </table>
        </div>

        <div class="chart-section">
          <div class="chart-title">Atividade Semanal</div>
          <table>
            <tr><th>Dia</th><th>Agendas</th></tr>
            ${atividadeSemanelCalculada.map(item => `<tr><td>${item.dia}</td><td>${item.agendas}</td></tr>`).join('')}
          </table>
        </div>
        ...
        <div class="chart-section">
          <div class="chart-title">Tabela de Remarcações</div>
          <table>
            <tr>
  ...
  const handleExport = (type) => {
    ...
    if (type === 'relatorio') {
      ...
      const blob = new Blob([JSON.stringify(relatorioData, null, 2)], { type: 'application/json' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `relatorio-${timestamp}.json`;
      a.click();
      URL.revokeObjectURL(url);
  ...
  // CORRIGIDO: Instruções de uso por página (mais concisas, com ícones)
  ...
  // CORRIGIDO: Rodapé com instruções mais concisas e ícones
  const Footer = () => (
  ...
  const Dashboard = () => (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* CORRIGIDO: Área aumentada do gráfico de prazos */}
        ...
        {/* CORRIGIDO: Gráfico de barras horizontais para melhor visibilidade */}
        <Card className="border-slate-200 shadow-sm">
          <CardHeader>
            <CardTitle className="text-lg font-medium text-slate-700 flex items-center gap-2">
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={topOfensores}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" opacity={0.3} />
                <XAxis 
                  dataKey="value"
                  tick={{ fontSize: 10, fill: '#64748b' }} 
                  domain={[0, Math.max(...topOfensores.map(d => d.value)) + 1]}
                />
                <YAxis 
                  dataKey="name"
      ...
      {/* CORRIGIDO: Atividade semanal calculada automaticamente */}
      <Card className="border-slate-200 shadow-sm">
        <CardHeader>
