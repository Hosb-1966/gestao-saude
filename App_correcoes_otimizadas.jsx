...
import { BarChart3, Calendar, Stethoscope, Repeat, Users, Clock, MapPin, Edit2, Save, X, Plus, Building2, Trash2, Phone, Timer, UserCheck, AlertTriangle, Download } from 'lucide-react';
...
function AppContent() {
  ...
  const remarcacoesStats = useMemo(() => {
    ...
    // Filtrar apenas "Dentro" e "Fora" para o gráfico de prazos - CORRIGIDO
  ...
  // Dados para gráfico de prazo (apenas "Dentro" e "Fora") - CORRIGIDO COM LEGENDAS
  ...
  const handleExport = (type) => {
    ...
    if (type === 'dashboard') {
      ...
      success('Dashboard exportado com sucesso!');
  ...
  // CORRIGIDO: Verificar se atividade semanal está lendo corretamente
  const handleEditAtividade = (index, value) => {
    ...
    success('Atividade semanal atualizada com sucesso!');
  };
  ...
  // Instruções de uso por página
  const getInstrucoesUso = () => {
    switch(view) {
      case 'dashboard':
        return "Dashboard: Visualize métricas gerais, gráficos dinâmicos e estatísticas em tempo real. Cards editáveis: clique no ícone de lápis para personalizar títulos.";
      case 'consultas':
        return "Consultas: Gerencie agendas por unidade e funcionário. Clique nas células para editar médicos. Use vírgulas para separar múltiplos médicos.";
      case 'exames':
        return "Exames: Controle tipos de exames por unidade e funcionário. Edite células clicando nelas. Separe tipos de exames com vírgulas.";
      case 'remarcacoes':
        return "Remarcações: Tabela editável completa. Use 'Adicionar Linha' para novas entradas. Cores automáticas para prazos 'Dentro' (verde) e 'Fora' (vermelho).";
      default:
        return "Sistema de Gestão de Saúde: Navegue pelas abas para acessar diferentes funcionalidades. Todos os dados são salvos automaticamente.";
    }
  };
  ...
  // Toolbar simplificada (removido busca, filtro, ajuda e configuração)
  const Toolbar = () => (
  ...
  const Footer = () => (
    <div className="bg-slate-100 border-t border-slate-200 px-6 py-4 mt-8">
      <div className="text-sm text-slate-600">
        <strong>Instruções de Uso:</strong> {getInstrucoesUso()}
      </div>
    </div>
  );
