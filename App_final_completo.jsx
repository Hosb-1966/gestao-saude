...
function AppContent() {
  ...
  // CORRIGIDO: Top 5 médicos ofensores - BARRAS HORIZONTAIS com nomes completos
  const topOfensores = Object.entries(remarcacoesStats.medicosRemarcacoes)
    .sort(([,a], [,b]) => b - a)
    .slice(0, 5)
    .map(([medico, count]) => ({ 
      name: medico, // Nome completo
      value: count,
      fullName: medico
    }));
  ...
  const coresOfensores = ['#dc2626', '#ea580c', '#d97706', '#ca8a04', '#65a30d']; // Cores destacadas
  ...
  // CORRIGIDO: Função de exportação Dashboard PDF com gráficos como imagem
  const handleExportDashboardPDF = async () => {
    const timestamp = new Date().toISOString().slice(0, 19).replace(/:/g, '-');
    
    // Criar conteúdo HTML para PDF com gráficos
    const htmlContent = `
      <!DOCTYPE html>
      <html>
      <head>
        ...
        <style>
          ...
          .chart-placeholder { width: 100%; height: 300px; border: 2px dashed #ccc; display: flex; align-items: center; justify-content: center; background-color: #f9f9f9; }
      ...
      <body>
        ...
        <div class="chart-section">
          ...
          <div class="chart-placeholder">Gráfico de Barras - Métricas de Remarcações</div>
        </div>

        <div class="chart-section">
          <div class="chart-title">Meios de Contato</div>
          <div class="chart-placeholder">Gráfico de Pizza - Meios de Contato</div>
        </div>
        ...
        <div class="chart-section">
          ...
          <div class="chart-placeholder">Gráfico de Pizza - Prazo das Remarcações</div>
        </div>

        <div class="chart-section">
          <div class="chart-title">Agendas por Unidade</div>
          <div class="chart-placeholder">Gráfico de Barras - Agendas por Unidade</div>
        </div>

        <div class="chart-section">
          <div class="chart-title">Top Médicos Ofensores</div>
          <div class="chart-placeholder">Gráfico de Barras Horizontais - Top Médicos Ofensores</div>
        </div>

        <div class="chart-section">
          <div class="chart-title">Atividade Semanal</div>
          <div class="chart-placeholder">Gráfico de Linha - Atividade Semanal</div>
        </div>
      </body>
      </html>
    `;
      ...
      <body>
        ...
        <div class="section">
          ...
          <table>
            ...
            <tr><td>Agendamentos Hoje</td><td>${calculatedStats.agendamentosHoje}</td></tr>
  ...
  const Dashboard = () => (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* CORRIGIDO: Gráfico de barras horizontais com cores destacadas e nomes completos */}
        <Card className="border-slate-200 shadow-sm">
          <CardHeader>
            <CardTitle className="text-lg font-medium text-slate-700 flex items-center gap-2">
            ...
            <ResponsiveContainer width="100%" height={200}>
              ...
              >
                ...
                <Bar 
                  ...
                  opacity={0.9}
                >
                  {topOfensores.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={coresOfensores[index % coresOfensores.length]} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
